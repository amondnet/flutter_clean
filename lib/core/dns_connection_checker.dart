...
final _logger = Logger('DnsConnectionChecker');

/// Represents the status of the data connection.
/// Returned by [DnsConnectionChecker.connectionStatus]
...
class DnsConnectionChecker {
  ...
  static final List<DnsOverHttps> DEFAULT_ADDRESSES = List.unmodifiable([
    DnsOverHttps.google(
  ...
  List<DnsOverHttps> addresses = DEFAULT_ADDRESSES;

  /// This is a singleton that can be accessed like a regular constructor
  /// i.e. DataConnectionChecker() always returns the same instance.
  factory DnsConnectionChecker() => _instance;

  DnsConnectionChecker._() {
  ...
  static final DnsConnectionChecker _instance = DnsConnectionChecker._();

  /// Ping a single address. See [AddressCheckOptions] for
  /// info on the accepted argument.
  Future<DnsCheckResult> isHostReachable(
    DnsOverHttps options,
  ) async {
    try {
      final result = await options.lookup('google.com');
      if (result.isNotEmpty) {
        _logger.finest('result : ${result.first.toString()}');
        return DnsCheckResult(options, true);
      } else {
        return DnsCheckResult(options, false);
      }
    } catch (e) {
  ...
  List<DnsCheckResult> get lastTryResults => _lastTryResults;
  List<DnsCheckResult> _lastTryResults = <DnsCheckResult>[];
  ...
  Future<bool> get hasConnection async {
    List<Future<DnsCheckResult>> requests = [];
...
/// default to [DnsConnectionChecker.DEFAULT_PORT]
/// and [DnsConnectionChecker.DEFAULT_TIMEOUT]
...
class AddressCheckOptions {
  ...
  AddressCheckOptions(
    this.address, {
    this.port = DnsConnectionChecker.DEFAULT_PORT,
    this.timeout = DnsConnectionChecker.DEFAULT_TIMEOUT,
  });
...
class DnsCheckResult {
  ...
  @override
  String toString() => "DnsCheckResult($client, $isSuccess)";
}
